...
//		}
		...
		return isRail(blockCheck.getType());
	}
...
//		}
		...
		return isIronBar(blockCheck.getType());
	}
...
//		}
		...
		return isCommandBlock(blockCheck.getType());
	}
...
//		}
		...
		return isRedstone(blockCheck.getType());
	}
...
//		}
		...
		return isSign(blockCheck.getType());
	}
			...
			for (BlockFace blockFace : FACES) {
				...
				if (b != null && b.getType() != null) {
					if (isRail(b) || isRedstone(b) || isCommandBlock(b) || isSign(b)) {
						...
						//Location a = new Location(loc .getWorld(), loc.getBlockX(), loc.getBlockY(), loc.getBlockZ());
						if (loc != null) {
	...
	{
		if (isRail(newCheckBlock)) {
			if (HideRailsManager.hr) {
				return true;
			}
		}
		else if (isIronBar(newCheckBlock)) {
			if (HideRailsManager.hb) {
				return true;
			}
		}
		else if (isCommandBlock(newCheckBlock)) {
			if (HideRailsManager.hc) {
				return true;
			}
		}
		else if (isRedstone(newCheckBlock)) {
			if (HideRailsManager.hd) {
				return true;
			}
		}
		else if (isSign(newCheckBlock)) {
	...
	public static BlockReplacementType getBlockReplacementType(Player player, Block targetBlock)
	{
		...
		if (isRail(targetBlock) && HideRailsManager.hr) {
			blockType = BlockReplacementType.RAIL;
		}
		else if (isIronBar(targetBlock) && HideRailsManager.hb) {
			blockType = BlockReplacementType.IRON_BAR;
		}
		else if (isCommandBlock(targetBlock) && HideRailsManager.hc) {
			blockType = BlockReplacementType.COMMAND_BLOCK;
		}
		else if (isRedstone(targetBlock) && HideRailsManager.hd) {
			blockType = BlockReplacementType.REDSTONE;
		}
		else if (isSign(targetBlock) && HideRailsManager.hs) {
